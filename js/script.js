...
// language change button & different languages tests
...
// Attaching handlers to each language selection button
langButtons.forEach((btn) => {
	btn.addEventListener("click", (event) => {
		userLang = event.target.dataset.btn;
		localStorage.setItem("language", event.target.dataset.btn);
		changeLang();
    // close menu after choose new language
    closeMenu();
	});
});


// Dropdown list 4 choose language
